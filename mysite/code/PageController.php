...
class PageController extends ContentController
{
	...
	protected function testDevServer()
	{
		if ($config = $this->getDevServerConfig()) {
			...
			if (isset($config['dockerhost']) && isset($config['port'])) {
	...
	{
		// Themed CSS Enabled?
        // var_dump($this->config()->themed_css);
        // die;
	...
	protected function loadThemedJS($name)
	{
		if ($this->isDevServerActive()) {
			$this->setDevThemedJS($name);
		} else {
			Requirements::themedJavascript($name);
		}
	}

	protected function setDevThemedJS($name)
	{
		if ($this->isDevServerActive()) {
			$this->devThemedJS = $this->getDevServerURL($this->ext($name, 'js'));
		}
	}

	public function getDevThemedJS()
	{
		return $this->devThemedJS;
	}
